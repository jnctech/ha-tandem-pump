...
# ═══════════════════════════════════════════════════════════════════════════
# Tests: Intermediate events queued for replay
# ═══════════════════════════════════════════════════════════════════════════


class TestHistoricalReplayQueueing:
    """Tests for intermediate event queueing."""

    async def test_intermediate_cgm_events_queued(self, hass: HomeAssistant):
        """Test that all but the latest CGM event are queued for replay."""
        events = [
            _make_cgm_event(seq=1, glucose_mgdl=100, minutes_ago=15),
            _make_cgm_event(seq=2, glucose_mgdl=110, minutes_ago=10),
            _make_cgm_event(seq=3, glucose_mgdl=120, minutes_ago=5),
            _make_cgm_event(seq=4, glucose_mgdl=130, minutes_ago=0),
        ]
        data = _make_pump_events_data(events)
        ...
        mock_client = AsyncMock()
        mock_client.login = AsyncMock(return_value=True)
        mock_client.get_recent_data = AsyncMock(return_value=data)
        mock_client.close = AsyncMock()
        ...
        # Call the internal parse method directly to inspect queue
        parsed = {}
        coordinator._parse_pump_events(events, parsed)

        # 3 intermediate CGM events should be queued (all except the latest)
        cgm_replays = [
            e for e in coordinator._pending_replay_events
            if e["event_id"] == 256
        ]
        assert len(cgm_replays) == 3

    async def test_single_event_no_replay(self, hass: HomeAssistant):
        """Test that a single event of each type does not trigger replay."""
        events = [
            _make_cgm_event(seq=1, glucose_mgdl=120),
            _make_bolus_completed_event(seq=2, insulin_delivered=2.0, iob=1.0),
            _make_basal_rate_change_event(seq=3, commanded_rate=0.8),
        ]
        data = _make_pump_events_data(events)
        ...
        assert len(coordinator._pending_replay_events) == 0

    async def test_replay_events_sorted_chronologically(
        self, hass: HomeAssistant
    ):
        """Test that queued replay events are sorted by timestamp."""
        events = [
            _make_cgm_event(seq=1, glucose_mgdl=100, minutes_ago=20),
            _make_basal_rate_change_event(seq=2, commanded_rate=0.8, minutes_ago=15),
            _make_cgm_event(seq=3, glucose_mgdl=110, minutes_ago=10),
            _make_basal_rate_change_event(seq=4, commanded_rate=1.0, minutes_ago=5),
            _make_cgm_event(seq=5, glucose_mgdl=120, minutes_ago=0),
            _make_basal_rate_change_event(seq=6, commanded_rate=1.2, minutes_ago=0),
        ]
        data = _make_pump_events_data(events)
        ...
        timestamps = [e["timestamp"] for e in coordinator._pending_replay_events]
        assert timestamps == sorted(timestamps)


# ═══════════════════════════════════════════════════════════════════════════
# Tests: _replay_historical_events
# ═══════════════════════════════════════════════════════════════════════════


class TestReplayHistoricalEvents:
    """Tests for the _replay_historical_events method."""

    async def test_replay_calls_async_set_updated_data(
        self, hass: HomeAssistant
    ):
        """Test that replay calls async_set_updated_data for each event."""
        events = [
            _make_cgm_event(seq=1, glucose_mgdl=100, minutes_ago=10),
            _make_cgm_event(seq=2, glucose_mgdl=110, minutes_ago=5),
            _make_cgm_event(seq=3, glucose_mgdl=120, minutes_ago=0),
        ]
        data = _make_pump_events_data(events)
        coordinator = await _setup_coordinator(hass, data)

        # After coordinator setup, the replay task should have been created.
        # Give the event loop a chance to process it.
        await hass.async_block_till_done()

        # The pending replay events should be cleared after replay
        assert coordinator._pending_replay_events == []

    async def test_replay_strips_readings_attributes(
        self, hass: HomeAssistant
    ):
        """Test that replay strips large readings arrays from attributes."""
        from custom_components.carelink import TandemCoordinator
        ...
        # Simulate base_data with readings attributes
        base_data = {
            DEVICE_PUMP_SERIAL: "12345678",
            DEVICE_PUMP_MODEL: "t:slim X2",
            DEVICE_PUMP_NAME: "Test",
            DEVICE_PUMP_MANUFACTURER: "Tandem Diabetes Care",
            f"{TANDEM_SENSOR_KEY_LASTSG_MGDL}_attributes": {
                "readings": [{"t": "2026-02-14T12:00:00", "v": 120}],
            },
        }

        # Set up pending events
        coordinator._pending_replay_events = [
            _make_cgm_event(seq=1, glucose_mgdl=100, minutes_ago=5),
        ]

        # Track what data is passed to async_set_updated_data
        replayed_data = []
        original_set = coordinator.async_set_updated_data

        def capture_set(data):
            replayed_data.append(dict(data))
            # Don't call original to avoid side effects in test
        coordinator.async_set_updated_data = capture_set

        await coordinator._replay_historical_events(base_data)

        assert len(replayed_data) == 1
        # The readings attribute should be stripped (empty dict)
        attr_key = f"{TANDEM_SENSOR_KEY_LASTSG_MGDL}_attributes"
        assert replayed_data[0][attr_key] == {}
