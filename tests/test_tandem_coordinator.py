...
class TestTandemCoordinatorFullData:
    ...
    async def test_device_info_from_metadata(self, hass: HomeAssistant, mock_tandem_recent_data):
    ...
    async def test_last_upload_parsed(self, hass: HomeAssistant, mock_tandem_recent_data):
    ...
    async def test_cgm_reading_parsed(self, hass: HomeAssistant, mock_tandem_recent_data):
    ...
    async def test_bolus_data_parsed(self, hass: HomeAssistant, mock_tandem_recent_data):
...
class TestTandemCoordinatorMinimalData:
    ...
    async def test_metadata_sensors_populated(self, hass: HomeAssistant, mock_tandem_recent_data_minimal):
        """Test that metadata sensors still work without ControlIQ data."""
        coordinator = await _setup_tandem_coordinator(hass, mock_tandem_recent_data_minimal)

        assert coordinator.data[TANDEM_SENSOR_KEY_PUMP_SERIAL_INFO] == "12345678"
        assert coordinator.data[TANDEM_SENSOR_KEY_PUMP_MODEL_INFO] == "t:slim X2"
        assert coordinator.data[TANDEM_SENSOR_KEY_SOFTWARE_VERSION] == "7.6.0"

    async def test_cgm_sensors_unavailable(self, hass: HomeAssistant, mock_tandem_recent_data_minimal):
    ...
    async def test_bolus_sensors_unavailable(self, hass: HomeAssistant, mock_tandem_recent_data_minimal):
    ...
    async def test_basal_sensors_unavailable(self, hass: HomeAssistant, mock_tandem_recent_data_minimal):
    ...
    async def test_summary_sensors_unavailable(self, hass: HomeAssistant, mock_tandem_recent_data_minimal):
...
class TestTandemCoordinatorSgDelta:
    ...
    async def test_delta_unavailable_on_first_update(self, hass: HomeAssistant, mock_tandem_recent_data):
